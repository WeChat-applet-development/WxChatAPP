...
class ThreadServiceImpl extends BaseService implements ThreadService
{
	...
	public function createPost($post)
	{
		...
		if (empty($thread)) {
			throw $this->createServiceException(sprintf('产品(ID: %s)话题(ID: %s)不存在。', $post['courseId'], $post['threadId']));
		}
	...
	public function deletePost($courseId, $id)
	{
		...
		if ($post['courseId'] != $courseId) {
			throw $this->createServiceException(sprintf('帖子#%s不属于产品#%s，删除失败。', $id, $courseId));
		}
