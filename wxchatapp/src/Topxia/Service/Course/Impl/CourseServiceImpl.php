...
class CourseServiceImpl extends BaseService implements CourseService
{
	...
	public function createCourse($course)
	{
		...
		$this->getLogService()->info('course', 'create', "创建产品《{$course['title']}》(#{$course['id']})");
	...
	public function updateCourse($id, $fields)
	{
		...
		$this->getLogService()->info('course', 'update', "更新产品《{$course['title']}》(#{$course['id']})的信息", $fields);
    ...
    public function changeCoursePicture($courseId, $data)
    {
		...
		$this->getLogService()->info('course', 'update_picture', "更新产品《{$course['title']}》(#{$course['id']})图片", $fields);
	...
	public function recommendCourse($id, $number)
	{
		...
		$this->getLogService()->info('course', 'recommend', "推荐产品《{$course['title']}》(#{$course['id']}),序号为{$number}");
	...
	public function hitCourse($id)
	{
		...
		if(empty($checkCourse)){
			throw $this->createServiceException("产品不存在，操作失败。");
		}
	...
	public function cancelRecommendCourse($id)
	{
		...
		$this->getLogService()->info('course', 'cancel_recommend', "取消推荐产品《{$course['title']}》(#{$course['id']})");
	}
	...
	public function deleteCourse($id)
	{
		...
		$this->getLogService()->info('course', 'delete', "删除产品《{$course['title']}》(#{$course['id']})");
	...
	public function publishCourse($id)
	{
		...
		$this->getLogService()->info('course', 'publish', "发布产品《{$course['title']}》(#{$course['id']})");
	}
	...
	public function closeCourse($id)
	{
		...
		$this->getLogService()->info('course', 'close', "关闭产品《{$course['title']}》(#{$course['id']})");
	}
	...
	public function hasFavoritedCourse($courseId)
	{
		...
		$course = $this->getCourse($courseId);
		if (empty($course)) {
			throw $this->createServiceException("产品{$courseId}不存在");
		}
	...
	public function setCoursePrice($courseId, $currency, $price)
	{
		...
		$course = $this->getCourse($courseId);
		if (empty($course)) {
			throw $this->createServiceException("产品不存在");
		}
	...
	public function createLesson($lesson)
	{
		...
		$this->getLogService()->info('course', 'add_lesson', "添加产品介绍《{$lesson['title']}》({$lesson['id']})", $lesson);
	...
	public function updateLesson($courseId, $lessonId, $fields)
	{
		...
		$this->getLogService()->info('course', 'update_lesson', "更新产品介绍《{$updatedLesson['title']}》({$updatedLesson['id']})", $updatedLesson);
	...
	public function deleteLesson($courseId, $lessonId)
	{
		...
		$this->getLogService()->info('lesson', 'delete', "删除产品《{$course['title']}》(#{$course['id']})的产品介绍 {$lesson['title']}");
	...
	{	
		...
		if (empty($course)) {
			throw $this->createServiceException('此产品不存在！');
		}
		...
		if ($thisLessons) {
			return array('error_occupied','该时段内已有直播产品介绍存在，请调整直播开始时间');
		}
	...
	public function canLearnLesson($courseId, $lessonId)
	{
		...
		return array('status' => 'no', 'message' => sprintf('本产品介绍需要%s学分才能浏览，您当前学分为%s分。', $lesson['requireCredit'], $member['credit']));
	}
	...
	public function createLessonView($createLessonView)
	{
		...
		$this->getLogService()->info('course', 'create', "{$this->getCurrentUser()->nickname}观看产品介绍《{$lesson['title']}》");
	...
	public function cancelLearnLesson($courseId, $lessonId)
	{
		...
		$learn = $this->getLessonLearnDao()->getLearnByUserIdAndLessonId($member['userId'], $lessonId);
		if (empty($learn)) {
			throw $this->createServiceException("产品介绍#{$lessonId}尚未浏览，取消浏览失败。");
		}

		if ($learn['status'] != 'finished') {
			throw $this->createServiceException("产品介绍#{$lessonId}尚未学完，取消浏览失败。");
		}
	...
	public function setCourseTeachers($courseId, $teachers)
	{
		...
		foreach (array_values($teachers) as $index => $teacher) {
			if (empty($teacher['id'])) {
				throw $this->createServiceException("商家ID不能为空，设置产品(#{$courseId})商家失败");
			}
			$user = $this->getUserService()->getUser($teacher['id']);
			if (empty($user)) {
				throw $this->createServiceException("用户不存在或没有商家角色，设置产品(#{$courseId})商家失败");
			}
		...
		$this->getLogService()->info('course', 'update_teacher', "更新产品#{$courseId}的商家", $teacherMembers);

		// 更新产品的teacherIds，该字段为产品可见商家的ID列表
	...
	/**
	 * @todo 当用户拥有大量的产品商家角色时，这个方法效率是有就有问题咯！鉴于短期内用户不会拥有大量的产品商家角色，先这么做着。
	 */
	public function cancelTeacherInAllCourses($userId)
	{
		...
		$this->getLogService()->info('course', 'cancel_teachers_all', "取消用户#{$userId}所有的产品商家角色");
	}
	...
	public function remarkStudent($courseId, $userId, $remark)
	{
		...
		if (empty($member)) {
			throw $this->createServiceException('产品顾客不存在，备注失败!');
		}
	...
	public function becomeStudent($courseId, $userId, $info = array())
	{
		...
		if($course['status'] != 'published') {
			throw $this->createServiceException('不能加入未发布产品');
		}
		...
		$member = $this->getMemberDao()->getMemberByCourseIdAndUserId($courseId, $userId);
		if ($member) {
			throw $this->createServiceException("用户(#{$userId})已加入该产品！");
		}
		...
		if (!empty($info['becomeUseMember'])) {
			...
			if ($levelChecked != 'ok') {
				throw $this->createServiceException("用户(#{$userId})不能以会员身份加入产品！");
			}
		...
		if (!empty($info['orderId'])) {
			...
			if (empty($order)) {
				throw $this->createServiceException("订单(#{$info['orderId']})不存在，加入产品失败！");
			}
		} else {
			$order = null;
		}
	...
	public function removeStudent($courseId, $userId)
	{
		$course = $this->getCourse($courseId);
		if (empty($course)) {
			throw $this->createNotFoundException("产品(#${$courseId})不存在，退出产品失败。");
		}

		$member = $this->getMemberDao()->getMemberByCourseIdAndUserId($courseId, $userId);
		if (empty($member) || ($member['role'] != 'student')) {
			throw $this->createServiceException("用户(#{$userId})不是产品(#{$courseId})的顾客，退出产品失败。");
		}
		...
		$this->getLogService()->info('course', 'remove_student', "产品《{$course['title']}》(#{$course['id']})，移除顾客#{$member['id']}");
	}
	...
	public function lockStudent($courseId, $userId)
	{
		$course = $this->getCourse($courseId);
		if (empty($course)) {
			throw $this->createNotFoundException("产品(#${$courseId})不存在，封锁顾客失败。");
		}

		$member = $this->getMemberDao()->getMemberByCourseIdAndUserId($courseId, $userId);
		if (empty($member) || ($member['role'] != 'student')) {
			throw $this->createServiceException("用户(#{$userId})不是产品(#{$courseId})的顾客，封锁顾客失败。");
		}
	...
	public function unlockStudent($courseId, $userId)
	{
		...
		$member = $this->getMemberDao()->getMemberByCourseIdAndUserId($courseId, $userId);
		if (empty($member) || ($member['role'] != 'student')) {
			throw $this->createServiceException("用户(#{$userId})不是产品(#{$courseId})的顾客，解封顾客失败。");
		}
	...
	public function tryManageCourse($courseId)
	{
		...
		if (!$this->hasCourseManagerRole($courseId, $user['id'])) {
			throw $this->createAccessDeniedException('您不是产品的商家或管理员，无权操作！');
		}
	...
	public function tryTakeCourse($courseId)
	{
		...
		if (empty($member) || !in_array($member['role'], array('teacher', 'student'))) {
			throw $this->createAccessDeniedException('您不是产品顾客，不能查看产品内容，请先购买产品！');
		}
	...
	public function isMemberNonExpired($course, $member)
	{
		...
		/*
		如果产品设置了限免时间，那么即使expiryDay为0，顾客到了deadline也不能参加浏览
		if ($course['expiryDay'] == 0) {
			return true;
		}
		*/
	...
	public function tryLearnCourse($courseId)
	{
		...
		if (empty($member) || !in_array($member['role'], array('admin', 'teacher', 'student'))) {
			throw $this->createAccessDeniedException('您不是产品顾客，不能浏览！');
		}
