...
class VipOrderProcessor extends BaseProcessor implements OrderProcessor
{
	...
	public function doPaySuccess($success, $order) {
        ...
        if ($order['data']['buyType'] == 'new') {
	        ...
	        $message = "您已经成功加入 {$level['name']} ，点击查看<a href='/vip/course/level/{$level['id']}' target='_blank'>{$level['name']}</a>产品";
	        ...
	        $level = $this->getLevelService()->getLevel($vip['levelId']);
	        $message = "您已经升级到 {$level['name']} ，点击查看<a href='/vip/course/level/{$level['id']}' target='_blank'>{$level['name']}</a>产品";
	    }
