...
class NeteaseOpenCourseAlbumParser extends AbstractAlbumParser
{
    ...
    {
        $matched = preg_match(iconv('utf-8', 'gb2312', '/class="m-cintro.*?<img\ssrc="(.*?)".*?\<h2\>(.*?)<\/h2>.*?本产品共(\d+).*?<p>产品介绍<\/p>.*?<p>(.*?)<\/p>/s'), $content, $matches);
        if (empty($matched)) {
            return null;
        }
    ...
    private function parseChineseAlbum($content)
    {
        ...
        $matched = preg_match( iconv('utf-8', 'gb2312', '/<h2>产品介绍<\/h2>.*?<span\sclass\=\"cdblan\">(.*?)<\/span>/s') , $content, $matches);
        if (empty($matched)) {
