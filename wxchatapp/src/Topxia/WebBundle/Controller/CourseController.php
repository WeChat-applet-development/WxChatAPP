...
class CourseController extends BaseController
{
	...
	/**
	 * 如果用户已购买了此产品，或者用户是该产品的商家，则显示产品的Dashboard界面。
	...
	{  
		...
		if ($type == 'live') {
			...
			if (empty($capacity['capacity']) && !empty($courseSetting['live_course_enabled'])) {
				return $this->createMessageResponse('info', '请联系平台官方购买直播教室，然后才能开启直播功能！');
			}
		}
	...
	public function exitAction(Request $request, $id)
	{
		...
		if (empty($member)) {
			throw $this->createAccessDeniedException('您不是产品的顾客。');
		}

		if ($member["joinedType"] == "course" && !empty($member['orderId'])) {
			throw $this->createAccessDeniedException('有关联的订单，不能直接退出查看。');
		}
	...
	public function learnAction(Request $request, $id)
	{
		...
		try{
			...
			throw $this->createAccessDeniedException('抱歉，未发布产品不能查看！');
		}
