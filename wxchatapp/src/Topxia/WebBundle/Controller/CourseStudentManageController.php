...
class CourseStudentManageController extends BaseController
{
	...
	public function createAction(Request $request, $id)
	{
		...
		if ('POST' == $request->getMethod()) {
			...
			if ($this->getCourseService()->isCourseStudent($course['id'], $user['id'])) {
				throw $this->createNotFoundException("用户已经是顾客，不能添加！");
			}

			$order = $this->getOrderService()->createOrder(array(
				'userId' => $user['id'],
				'title' => "购买产品《{$course['title']}》(管理员添加)",
				'targetType' => 'course',
				'targetId' => $course['id'],
				'amount' => $data['price'],
				'payment' => 'none',
			...
			$this->getLogService()->info('course', 'add_student', "产品《{$course['title']}》(#{$course['id']})，添加顾客{$user['nickname']}(#{$user['id']})，备注：{$data['remark']}");
	...
	{   
		...
		$str = "用户名,Email,加入查看时间,查看进度,姓名,性别,QQ号,微信号,手机号,公司,职业,头衔";
	...
	public function showAction(Request $request, $courseId, $userId)
	{
		if (!$this->getCurrentUser()->isAdmin()) {
			throw $this->createAccessDeniedException('您无权查看顾客详细信息！');
		}
