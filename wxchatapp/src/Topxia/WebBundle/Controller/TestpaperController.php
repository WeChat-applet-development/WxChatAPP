...
class TestpaperController extends BaseController
{
    ...
    public function doTestpaperAction (Request $request, $testId)
    {
        ...
        if ($targets[$testpaper['target']]['type'] != 'course') {
            throw $this->createAccessDeniedException('调查问卷只能属于产品');
        }

        $course = $this->getCourseService()->getCourse($targets[$testpaper['target']]['id']);

        if (empty($course)) {
            return $this->createMessageResponse('info', '调查问卷所属产品不存在！');
        }

        if (!$this->getCourseService()->canTakeCourse($course)) {
            return $this->createMessageResponse('info', '不是调查问卷所属产品商家或顾客');
        }
        ...
        if (empty($testpaperResult)) {
            ...
            if ($testpaper['status'] == 'draft') {
                return $this->createMessageResponse('info', '该调查问卷未发布，如有疑问请联系商家！');
            }
            if ($testpaper['status'] == 'closed') {
                return $this->createMessageResponse('info', '该调查问卷已关闭，如有疑问请联系商家！');
            }
    ...
    public function previewTestAction (Request $request, $testId)
    {
        ...
        if (!$teacherId = $this->getTestpaperService()->canTeacherCheck($testpaper['id'])){
            throw $this->createAccessDeniedException('无权预览调查问卷！');
        }
    ...
    public function showTestAction (Request $request, $id)
    {
        ...
        if (!$testpaperResult) {
            throw $this->createNotFoundException('调查问卷不存在!');
        }
        if ($testpaperResult['userId'] != $this->getCurrentUser()->id) {
            throw $this->createAccessDeniedException('不可以访问其他顾客的调查问卷哦~');
        }
        ...
        if ($request->getMethod() == 'POST') {
            ...
            if ($this->getTestpaperService()->isExistsEssay($testResults)) {
                ...
                foreach ($course['teacherIds'] as $receiverId) {
                    $result = $this->getNotificationService()->notify($receiverId, 'default', "【调查问卷已完成】 <a href='{$userUrl}' target='_blank'>{$user['nickname']}</a> 刚刚完成了 {$testpaperResult['paperName']} ，<a href='{$teacherCheckUrl}' target='_blank'>请点击批阅</a>");
                }
            }
    ...
    public function teacherCheckAction (Request $request, $id)
    {
        ...
        if (!$teacherId = $this->getTestpaperService()->canTeacherCheck($testpaper['id'])){
            throw $this->createAccessDeniedException('无权批阅调查问卷！');
        }
        ...
        if ($request->getMethod() == 'POST') {
            ...
            $result = $this->getNotificationService()->notify($testpaperResult['userId'], 'default', "【调查问卷已批阅】 <a href='{$userUrl}' target='_blank'>{$user['nickname']}</a> 刚刚批阅了 {$testpaperResult['paperName']} ，<a href='{$testpaperResultUrl}' target='_blank'>请点击查看结果</a>");
    ...
    public function listReviewingTestAction (Request $request)
    {
        ...
        if(!$user->isTeacher()) {
            return $this->createMessageResponse('error', '您不是商家，不能查看此页面！');
        }
    ...
    public function userResultJsonAction(Request $request, $id)
    {
        ...
        if (empty($testpaper)) {
            return $this->createJsonResponse(array('error' => '调查问卷已删除，请联系管理员。'));
        }
