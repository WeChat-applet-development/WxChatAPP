...
class CourseSettingController extends BaseController
{
    public function courseSettingAction(Request $request)
    {
        ...
        $default = array(
            ...
            'welcome_message_body' => '{{nickname}},欢迎加入产品{{course}}',
            'buy_fill_userinfo' => '0',
    ...
    public function courseAvatarAction(Request $request)
    {
        ...
        if ($request->getMethod() == 'POST') {
            ...
            $this->getLogService()->info('system', 'update_settings', "更新产品默认图片设置", $defaultSetting);
            $this->setFlashMessage('success', '产品默认图片设置已保存！');
        }
    ...
    public function liveCourseSettingAction(Request $request)
    {
        ...
        if ($request->getMethod() == 'POST') {
            ...
            $this->getLogService()->info('system', 'update_settings', "更新产品设置", $setting);
            $this->setFlashMessage('success', '产品设置已保存！');
        }
