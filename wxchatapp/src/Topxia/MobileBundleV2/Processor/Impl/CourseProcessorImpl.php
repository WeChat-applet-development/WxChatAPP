...
class CourseProcessorImpl extends BaseProcessor implements CourseProcessor
{
    ...
    public function updatePost()
    {
        ...
        $user = $this->controller->getUserByToken($this->request);
        if (!$user->isLogin()) {
            return $this->createErrorResponse('not_login', "您尚未登录，不能评价产品！");
        }
    ...
    public function updateThread()
    {
        ...
        $user = $this->controller->getUserByToken($this->request);
        if (!$user->isLogin()) {
            return $this->createErrorResponse('not_login', '您尚未登录，修改该产品介绍');
        }
    ...
    public function commitCourse()
    {
        ...
        if (!$user->isLogin()) {
            return $this->createErrorResponse($request, 'not_login', "您尚未登录，不能评价产品！");
        }
        
        $course = $this->controller->getCourseService()->getCourse($courseId);
        if (empty($course)) {
            return $this->createErrorResponse('not_found', "产品#{$courseId}不存在，不能评价！");
        }
        
        if (!$this->controller->getCourseService()->canTakeCourse($course)) {
            return $this->createErrorResponse('access_denied', "您不是产品《{$course['title']}》顾客，不能评价产品！");
        }
    ...
    public function getThreadPost()
    {
        ...
        $user = $this->controller->getUserByToken($this->request);
        if (!$user->isLogin()) {
            return $this->createErrorResponse('not_login', '您尚未登录，不能查看该产品介绍');
        }
    ...
    public function favoriteCourse()
    {
        ...
        if (empty($user) || !$user->isLogin()) {
            return $this->createErrorResponse('not_login', "您尚未登录，不能收藏产品！");
        }
    ...
    public function unFavoriteCourse()
    {
        ...
        if (!$this->controller->getCourseService()->hasFavoritedCourse($courseId)) {
            return $this->createErrorResponse('runtime_error', "您尚未收藏产品，不能取消收藏！");
        }
    ...
    public function vipLearn()
    {
        if (!$this->controller->setting('vip.enabled')) {
            return $this->createErrorResponse('error', "平台没有开启vip功能");
        }
    ...
    public function unLearnCourse()
    {
        ...
        if (empty($member)) {
            return $this->createErrorResponse('not_member', '您不是产品的顾客或尚未购买该产品，不能退学。');
        }
    ...
    public function getCourse()
    {
        ...
        if (empty($course)) {
            return $this->createErrorResponse('not_found', "产品不存在");
        }
        
        $member = $user->isLogin() ? $this->controller->getCourseService()->getCourseMember($course['id'], $user['id']) : null;
        $member = $this->previewAsMember($member, $courseId, $user);
        if ($member && $member['locked']) {
            return $this->createErrorResponse('member_locked', "会员被锁住，不能访问产品，请联系管理员!");
        }
        if ($course['status'] != 'published') {
            if (!$user->isLogin()) {
                return $this->createErrorResponse('course_not_published', "产品未发布或已关闭。");
            }
            if (empty($member)) {
                return $this->createErrorResponse('course_not_published', "产品未发布或已关闭。");
            }
