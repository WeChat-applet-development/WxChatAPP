...
class UserController extends MobileController
{
    ...
    public function loginWithTokenAction(Request $request)
    {
        ...
        if (empty($mobile['enabled'])) {
            return $this->createErrorResponse($request, 'client_closed', '没有搜索到该平台！');
        }
     ...
     private function coverNotifyContent($notification)
    {
        ...
        switch ($type) {
            case 'thread-post':
                ...
                break;
            case 'thread':
                $message = $notification["content"]["threadUserNickname"] . " 产品 " .  $notification["content"]["courseTitle"] . "发表了问题 " . $notification["content"]["threadTitle"] ;
                break;
