...
class CourseController extends MobileController
{
    ...
    public function coursesAction(Request $request)
    {
		...
		$result['count'] = (int) $request->query->get('count', 10);
        ...
        $result['posts'] = $courses = $this->filterCourses($courses);
    ...
    public function courseAction(Request $request, $courseId)
    {
        ...
        if (empty($course)) {
            $error = array('error' => 'not_found', 'message' => "产品#{$courseId}不存在。");
            return $this->createJson($request, $error);
        }

        if ($course['status'] != 'published') {
            $error = array('error' => 'course_not_published', 'message' => "产品#{$courseId}未发布或已关闭。");
        }
        ...
        if ($course) {
            $this->getLogService()->info(MobileController::MOBILE_MODULE, "view_course", "浏览产品",  array(
                "courseId" => $course["id"],
                "title" => $course["title"]
                )
            );
    ...
    public function lessonAction(Request $request, $courseId, $lessonId)
    {
        ...
        if (!$user->isLogin()) {
            return $this->createErrorResponse($request, 'not_login', '您尚未登录，不能查看产品介绍！');
        }
    ...
    public function favoriteAction(Request $request, $courseId)
    {
        ...
        if (!$user->isLogin()) {
            return $this->createErrorResponse($request, 'not_login', "您尚未登录，不能收藏产品！");
        }
    ...
    public function unfavoriteAction(Request $request, $courseId)
    {
        ...
        if (!$this->getCourseService()->hasFavoritedCourse($courseId)) {
            return $this->createErrorResponse($request, 'runtime_error', "您尚未收藏产品，不能取消收藏！");
        }
    ...
    public function canLearnAction(Request $request, $courseId)
    {
        ...
        if (!$user->isLogin()) {
            $result = array('status' => 'fail', 'message' => '您尚未登录，不能浏览');
            goto response;
        }

        if (!$this->getCourseService()->isCourseStudent($courseId, $user['id'])) {
            $result = array('status' => 'fail', 'message' => '您不是产品顾客，不能浏览');
            goto response;
        }
