...
class CourseReviewController extends MobileController
{
    ...
    public function createAction(Request $request, $courseId)
    {
        ...
        if (empty($course)) {
            return $this->createErrorResponse($request, 'not_found', "产品#{$courseId}不存在，不能评价！");
        }

        if (!$this->getCourseService()->canTakeCourse($course)) {
            return $this->createErrorResponse($request, 'access_denied', "您不是产品《{$course['title']}》顾客，不能评价产品！");
        }
