...
class MobileController extends BaseController
{
    ...
    public function mobileSchoolLoginAction(Request $request)
    {
        ...
        $this->getLogService()->info(MobileController::MOBILE_MODULE, "school_login", "APP移动端登录",  $parames);
    ...
    protected function createJson(Request $request, $data)
    {
        if (empty($data['error'])) {
            ...
            if (empty($mobile['enabled'])) {
                return $this->createErrorResponse($request, 'client_closed', 'APP移动端客户端功能已关闭');
            }
        }
